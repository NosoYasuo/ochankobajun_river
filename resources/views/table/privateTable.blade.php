<table class="table table-borderless d-flex align-items-center w-100">
  <tbody class="card-body w-100" style="display:flex; flex-direction: column; padding: 0;">
    ...
    <tr class="card mx-auto w-100 px-1 py-2" style="border: none; border-bottom: 0.5px solid #81D8D0">
      ...
      <td class="justify-content-center">
        <div class="d-flex justify-content-between">
          ...
          @endguest
          {{-- コメント表示 --}}
          <div class="d-flex" id=p{{$post->id}}>
            <div><i class="far fa-comment-alt"></i> コメント</div>
            <div>{{$post->comments_count}} 件</div>
          </div>
        </div>
        <div id="p_commentArea{{$post->id}}" style="display: none" class="px-4 mt-4">
        ...
        @if (!Request::is('mypage'))
        <form id="p_commentForm{{$post->id}}" style="display: none" action="{{ url('comment') }}" method="POST" class="form-horizontal px-4">
    ...
    @if (count ($privates) < 3) @if (count ($privates)==1) @include('youtube') @else @if ($loop->iteration == 2)
      @include('youtube')
      @endif
      @endif
      @else
...
<script>
  let privates = @json($privates);
  let privateId = privates.map(private => private.id)
  privateId.forEach((id) => {
    document.getElementById("p" + id).addEventListener('click', () => {
      //style="display: none"
      document.getElementById("p_commentArea" + id).style.display = 'block';
      document.getElementById("p_commentForm" + id).style.display = 'block';
    })
